...
1. 本地 `npm run dev` 检查效果 → 提交推送 → 自动上线。

> 想隐藏某篇文章不发布？在 frontmatter 加 
>
> `draft: true`
>
>  即可。
...
> **部署架构**
>
> （适配 VPS 已有主机 Caddy 占用 80/443）：
> 博客容器只在 VPS 内网 
>
> `127.0.0.1:8090`
>
>  提供 HTTP 静态服务；
> VPS 主机上的 Caddy（负责你其他服务的那台）把 
>
> `howdo.icu`
>
>  反向代理到该端口并自动签发 HTTPS。
...
* **GitHub Actions 部署失败？** 检查三个 Secret 是否填写正确、VPS 是否放行了 22 端口、`VPS_SSH_KEY` 私钥与 authorized\_keys 公钥是否配对。

* **报错 "can't connect without a private SSH key or password"？** 说明 `VPS_SSH_KEY` 这个 Secret 是空的：确认它是在仓库 → Settings → Secrets and variables → **Actions** 下添加（不是 Codespaces/Dependabot），且名字严格为 `VPS_SSH_KEY`。
